...
class CCT_Extractor(nn.Module):
    def __init__(self, args) -> None:
        super(CCT_Extractor, self).__init__()
        self.args = args
        self.cct = CCT(img_size = 224, num_frames = 64,
                        n_input_channels=3,
                        embedding_dim = 384, n_conv_layers = 2,
                        frame_kernel_size = 7, kernel_size = 7,
                        stride = 2, padding = 3, pooling_kernel_size = 3,
                        pooling_stride = 2, pooling_padding = 1,
                        num_layers = 14, num_heads = 6, mlp_ratio = 3.,
                        num_classes = 512, positional_embedding = 'learnable')
...
if __name__=="__main__":
    model = CCT_Extractor(1)
    x = torch.zeros((2,3,64,224,224))
    y = model(x)
    print(y.shape)
