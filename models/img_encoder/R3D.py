...
class R3DClassifier(nn.Module):
    ...
    def __init__(self, args):
        ...
        if args.net_backbone.lower() == 'resnet18':
            self.res3d = R3DNet(layer_sizes=(2,2,2,2), block_type=SpatioTemporalResBlock)
        elif args.net_backbone.lower() == 'resnet50':
            self.res3d = R3DNet(layer_sizes=(3,4,6,3), block_type=SpatioTemporalResBlock)
        else:
            raise NotImplementedError("Backbone {} is not implemented!".format(args.net_backbone))
        self.classifier = MHCLS(args)

        self.__init_weight()
        self.pretrained = nn.ModuleList([self.res3d])
