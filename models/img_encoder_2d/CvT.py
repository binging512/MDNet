...
class CvT_Extractor(nn.Module):
    def __init__(self, args) -> None:
        ...
        if args.net_backbone.lower() in ['cvt-13','cvt13']:
            cvt_spec = cvt_13_spec
        elif args.net_backbone.lower() in ['cvt-21','cvt21']:
            cvt_spec = cvt_21_spec
        elif args.net_backbone.lower() in ['cvt-w24','cvtw24']:
            cvt_spec = cvt_w24_spec
        else:
            raise NotImplementedError('Backbone {} is not implemented!!!'.format(args.net_backbone))
        
        self.cvt_model = ConvolutionalVisionTransformer(
            in_chans=3,
            num_classes=2,
            act_layer=QuickGELU,
            norm_layer=partial(LayerNorm, eps=1e-5),
            init=getattr(cvt_spec, 'INIT', 'trunc_norm'),
            spec=cvt_spec,
            )
        
        self.proj = nn.Sequential(
            nn.Linear(cvt_spec['DIM_EMBED'][-1], 512),
            nn.LayerNorm(512),
            nn.GELU()
        )
        
        self.pretrained = nn.ModuleList([self.cvt_model])
        self.new_added = nn.ModuleList([self.proj])

        if args.net_pretrain:
            pretrain_state_dict = torch.load(args.net_pretrain)
            model_state_dict = self.cvt_model.state_dict()
            new_state_dict = {}
            ...
            self.cvt_model.load_state_dict(new_state_dict)
            print("Pretrained model loaded from: {}".format(args.net_pretrain))
...
if __name__=="__main__":
    ...
    parser.add_argument('--net_seg_celoss', default=False, type=bool)
    parser.add_argument('--net_backbone', default='cvt-13', type=str)
    parser.add_argument("--net_num_classes", default=2, type=int)
    parser.add_argument("--net_pretrain", default="pretrained/CvT-13-384x384-IN-22k.pth", type=str)
    
    args = parser.parse_args()
    
    model = CvT_Extractor(args)
