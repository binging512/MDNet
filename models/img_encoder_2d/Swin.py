...
class SwinTransformer_Decoder(nn.Module):
    def __init__(self, args, in_channels) -> None:
        super(SwinTransformer_Decoder, self).__init__()
    ...
    def forward(self, feat_list):
        ...
        feat1 = F.interpolate(feat1, scale_factor=2, mode='bilinear') # C/2 x H/16 x W/16
...
class SwinTransformer_Extractor(nn.Module):
    def __init__(self, args) -> None:
        super(SwinTransformer_Extractor, self).__init__()
        if args.net_backbone.lower() in ['base', 'b']:
            ...
            self.layer = nn.Sequential(nn.Linear(1024, 512),
                                        nn.BatchNorm1d(512),
                                        nn.ReLU(inplace=True))
            self.decoder = SwinTransformer_Decoder(args, in_channels=1024)
        else:
        ...
        if args.net_pretrain:
            ...
            pretrained_state_dict = torch.load(args.net_pretrain)['state_dict']
            new_state_dict = {}
            for k,v in state_dict.items():
                if 'backbone.'+k in list(pretrained_state_dict.keys()):
                    if v.shape == pretrained_state_dict['backbone.'+k].shape:
                        new_state_dict[k] = pretrained_state_dict['backbone.'+k]
            self.swin.load_state_dict(new_state_dict, strict=False)
            
        self.pretrained = nn.ModuleList([self.swin])
        self.new_added = nn.ModuleList([self.layer, self.decoder])
        
    def forward(self,x):
        feat_list= self.swin(x)
        
        feat = self.maxpool(feat_list[-1]).squeeze(2).squeeze(2)
