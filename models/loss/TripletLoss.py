...
def hard_example_mining(dist_mat, labels, return_inds=False):
    ...
    temp_dist_mat[is_neg] = 0
    print(temp_dist_mat)
    print(dist_mat)
    dist_ap, relative_p_inds = torch.max(temp_dist_mat, 1, keepdim=True)
    
    # `dist_an` means distance(anchor, negative)
    # both `dist_an` and `relative_n_inds` with shape [N, 1]
    # dist_an, relative_n_inds = torch.min(dist_mat[is_neg].contiguous().view(N, -1), 1, keepdim=True)
    temp_dist_mat = dist_mat
    temp_dist_mat[is_pos] = 99999
    dist_an, relative_n_inds = torch.max(temp_dist_mat, 1, keepdim=True)
    # shape [N]
